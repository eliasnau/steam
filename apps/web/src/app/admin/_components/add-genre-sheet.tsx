...
const addGenreMutation = async (data: { name: string }) => {
	const result = await client.genres.create(data);
	return result;
};

interface AddGenreSheetProps {
	trigger?: ReactNode;
	renderTrigger?:
		| React.ReactElement
...
export default function AddGenreSheet({
	trigger,
	renderTrigger,
	open,
	onOpenChange,
	...
	const mutation = useMutation({
		mutationFn: addGenreMutation,
		onSuccess: (data) => {
			toast.success(`Genre "${data.genre.name}" erfolgreich erstellt!`);
			form.reset();
			onCreated?.(data.genre);
			if (onOpenChange) {
				onOpenChange(true);
			}
		},
		onError: (error: Error) => {
			let errorMessage = "Das Genre konnte nicht erstellt werden";
	...
	return (
		<Sheet open={open} onOpenChange={onOpenChange}>
			...
			<SheetPopup side="right" inset>
				...
				>
					<SheetHeader>
						<SheetTitle>Neues Genre erstellen</SheetTitle>
						<SheetDescription>
							Füge ein neues Genre zur Datenbank hinzu.
						</SheetDescription>
					</SheetHeader>
					<SheetPanel>
